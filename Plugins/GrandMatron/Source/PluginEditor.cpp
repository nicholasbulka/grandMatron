...
//==============================================================================
ResponseCurveComponent::ResponseCurveComponent(GrandMatronAudioProcessor& p) :
  audioProcessor(p),
...
void ResponseCurveComponent::updateResponseCurve()
{
  ...
  for( int i = 0; i < w; ++i )
  {
      ...
      if( !monoChain.isBypassed<ChainPositions::highPass>() )
      {
      ...
      if( !monoChain.isBypassed<ChainPositions::lowPass>() )
      {
...
void ResponseCurveComponent::updateChain()
{
  ...
  monoChain.setBypassed<ChainPositions::highPass>(chainSettings.highPassBypassed);
  monoChain.setBypassed<ChainPositions::Peak>(chainSettings.peakBypassed);
  monoChain.setBypassed<ChainPositions::lowPass>(chainSettings.lowPassBypassed);
  ...
  updateCutFilter(monoChain.get<ChainPositions::highPass>(),
                  highPassCoefficients,
                  chainSettings.highPassSlope);

  updateCutFilter(monoChain.get<ChainPositions::lowPass>(),
                  lowPassCoefficients,
...
GrandMatronAudioProcessorEditor::GrandMatronAudioProcessorEditor (GrandMatronAudioProcessor& p)
  ...
  highPassBypassButtonAttachment(audioProcessor.apvts, "highPass Bypassed", highPassBypassButton),
  peakBypassButtonAttachment(audioProcessor.apvts, "Peak Bypassed", peakBypassButton),
  lowPassBypassButtonAttachment(audioProcessor.apvts, "lowPass Bypassed", lowPassBypassButton),
  analyzerEnabledButtonAttachment(audioProcessor.apvts, "Analyzer Enabled", analyzerEnabledButton)
{
  ...
  highPassSlopeSlider.labels.add({0.0f, "12"});
  ...
  auto safePtr = juce::Component::SafePointer<GrandMatronAudioProcessorEditor>(this);
  ...
  highPassBypassButton.onClick = [safePtr]()
  {
      if( auto* comp = safePtr.getComponent() )
      {
          ...
          comp->highPassFreqSlider.setEnabled( !bypassed );
  ...
  lowPassBypassButton.onClick = [safePtr]()
  {
      if( auto* comp = safePtr.getComponent() )
      {
          ...
          comp->lowPassFreqSlider.setEnabled( !bypassed );
...
void GrandMatronAudioProcessorEditor::paint(juce::Graphics &g)
{
  ...
  g.setFont(14);
  g.drawFittedText("highPass", highPassSlopeSlider.getBounds(), juce::Justification::centredBottom, 1);
  g.drawFittedText("Peak", peakQualitySlider.getBounds(), juce::Justification::centredBottom, 1);
  g.drawFittedText("lowPass", lowPassSlopeSlider.getBounds(), juce::Justification::centredBottom, 1);

  auto buildDate = Time::getCompilationDate().toString(true, false);
  auto buildTime = Time::getCompilationDate().toString(false, true);
  g.setFont(12);
  g.drawFittedText("Build: " + buildDate + "\n" + buildTime, lowPassSlopeSlider.getBounds().withY(6), Justification::topRight, 2);
}
...
void GrandMatronAudioProcessorEditor::resized()
{
  ...
  highPassBypassButton.setBounds(highPassArea.removeFromTop(25));
  ...
  lowPassBypassButton.setBounds(lowPassArea.removeFromTop(25));
